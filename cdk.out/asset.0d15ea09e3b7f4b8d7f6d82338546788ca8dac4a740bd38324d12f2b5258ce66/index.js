...
var client = new import_client_dynamodb.DynamoDBClient({ region: "your-region" });
var seed = async () => {
  const product1Id = v4_default();
  const product2Id = v4_default();
  ...
  const stock = formattedProducts.map((product, index) => ({
    ...
    count: { N: Math.round(product.price / index) }
  }));
